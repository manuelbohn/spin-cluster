...
llh <- webppl(
  ...
  chains = 2,
  cores = 2,
  inference_opts = list(method = "incrementalMH", samples = 3, lag = 4, burn = 1, verbose = T, verboseLag = 1)
)
end_time <- Sys.time()
...
llh %>%
  separate(Parameter,c("type","model","age","item","condition"),sep =",")%>%
  group_by(Chain, model, Iteration)%>%
  summarise(loglike_sample = sum(value))%>%
